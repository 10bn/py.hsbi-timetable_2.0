...
def save_raw_tables(table_list: List[camelot.core.Table], output_dir: str) -> None:
    """
    ...
    logger.info(f"Saving {len(table_list)} raw tables to directory: {raw_output_dir}")
    ...
    for idx, table in enumerate(table_list, start=1):
        ...
        try:
            ...
            logger.error(f"Failed to save raw table {idx} to '{table_filename}': {e}", exc_info=True)

    logger.info(f"All raw tables have been saved to '{raw_output_dir}'.")


def convert_tablelist_to_dataframe(table_list: List[camelot.core.Table]) -> pd.DataFrame:
    """
    ...
    try:
        ...
        logger.error(f"Error converting tables to DataFrame: {e}", exc_info=True)
        return pd.DataFrame()
...
def melt_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    try:
        logger.info("Melting DataFrame to long format with 'date', 'time_slot', and 'raw_details'.")
        melted_df = df.melt(id_vars=["date"], var_name="time_slot", value_name="raw_details")
...
def clean_time_slot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "time_slot" in df.columns:
        try:
            ...
            logger.error(f"Error cleaning 'time_slot' column: {e}", exc_info=True)
    else:
...
def split_time_slot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        df["start_time"] = pd.to_datetime(time_splits["start_time_str"].str.strip(), format="%H.%M", errors="coerce").dt.time
        df["end_time"] = pd.to_datetime(time_splits["end_time_str"].str.strip(), format="%H.%M", errors="coerce").dt.time
        ...
        if start_time_issues.any():
            ...
            logger.warning(f"Failed to parse 'start_time' for entries: {problematic_slots}")

        if end_time_issues.any():
            problematic_slots = df.loc[end_time_issues, "time_slot"].tolist()
            logger.warning(f"Failed to parse 'end_time' for entries: {problematic_slots}")

        # Drop the original 'time_slot' column
        df.drop("time_slot", axis=1, inplace=True)
        logger.debug("'time_slot' column has been replaced with 'start_time' and 'end_time'.")
    except Exception as e:
...
def format_date(df: pd.DataFrame, current_year: int) -> pd.DataFrame:
    ...
    try:
        ...
        if df["date"].isna().any():
            ...
            logger.warning(f"Some dates could not be parsed and are set to NaT: {failed_dates}")
...
def validate_dates(df: pd.DataFrame, start_year: int, end_year: int) -> pd.DataFrame:
    """
...
def get_year(pdf_path: str) -> Optional[int]:
    ...
    try:
        ...
        if isinstance(version_data, str):
            logger.debug("Extracted version data is a string. Attempting regex search for year.")
            ...
            if year_match:
                ...
                logger.warning(f"No valid year found in version string: '{version_data}'")
                return None
            ...
            logger.warning("Version data is neither a string nor a datetime object.")
            return None
    except Exception as e:
        logger.error(f"Error extracting year from PDF '{pdf_path}': {e}", exc_info=True)
        return None
...
def openai_parser(api_key: str, details: str) -> List[Dict[str, Any]]:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if not structured_response:
                logger.warning("Received empty response from OpenAI. Retrying...")
                continue
            ...
            if isinstance(structured_data, dict):
                ...
                logger.warning("Parsed data is neither a list nor a dict. Returning failure response.")
                return failure_response

        except json.JSONDecodeError as e:
            logger.warning(f"Attempt {attempt}: JSON decode error: {e}. Retrying...")
        except openai.error.OpenAIError as e:
            logger.error(f"Attempt {attempt}: OpenAI API error: {e}. Retrying...")
        except Exception as e:
            logger.error(f"Attempt {attempt}: Unexpected error: {e}. Retrying...", exc_info=True)
    ...
    logger.critical("Failed to parse details using OpenAI after multiple attempts.")
    return failure_response
...
def convert_raw_event_data_to_list(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    try:
        logger.info("Converting 'raw_details' from strings to lists and cleaning special characters.")
        ...
        logger.error(f"Error converting 'raw_details' to lists: {e}", exc_info=True)
    return df
...
def check_multievent(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        df["multi_event"] = df["raw_details"].apply(lambda x: isinstance(x, list) and len(x) > 4)
...
def process_data(df: pd.DataFrame, api_key: str) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        ...
        if row.get("multi_event"):
            logger.info(f"Row {index} identified as multi-event. Invoking OpenAI parser.")
            ...
            for event in parsed_events:
                if isinstance(event, dict):
                    processed_event = {
                        ...
                        "lecturer": event.get("lecturer", ["Unknown Lecturer"]),
                        "location": event.get("location", "Unknown Location"),
                        "details": event.get("details", ""),
                    }
                    processed_events.append(processed_event)
                    logger.info(f"Added parsed event from row {index}: {processed_event}")
                else:
                    logger.warning(f"Unexpected event format in row {index}: {event}")
        else:
            if isinstance(raw_details, list):
                ...
                logger.warning(f"Expected 'raw_details' to be a list in row {index}, got {type(raw_details)}.")

    processed_df = pd.DataFrame(processed_events, columns=processed_events_columns)
    logger.info(f"Processing completed. Total processed events: {len(processed_df)}")
    return processed_df
...
class PdfParser:
    ...
    def __init__(
        ...
        logger.debug(f"PdfParser initialized with output directory: {self.output_dir}")
    ...
    def parse_pdf(
        ...
        if df.empty:
            logger.error("Conversion resulted in an empty DataFrame. Aborting processing.")
            return None
        ...
        if save_csv_events:
            ...
            try:
                ...
                logger.info(f"Processed events saved to '{output_csv_path}'.")
            except Exception as e:
                logger.error(f"Failed to save processed events to CSV: {e}", exc_info=True)
...
def main():
    ...
    pdf_path = "downloads/timetable_elm_3/2024-09-26 11:13:00/Stundenplan WS_2024_2025_ELM 3.pdf"
